package com.cyh.bigimageloader.view;
        ...
        public void zoom(float factor, PointF screenFocus) {
            if (factor != 1.0) {
                ...
                synchronized (this) {
                    ...
                    RectF w2 = new RectF();
                    PointF sceneFocus = new PointF(w1.left + (screenFocus.x / screenSize.x) * w1.width(), w1.top + (screenFocus.y / screenSize.y) * w1.height());
    ...
    private class Cache {
        ...
        void loadSampleIntoViewport() {
            if (getState() != CacheState.UNINITIALIZED) {
                synchronized (viewport) {
                    drawSampleRectIntoBitmap(viewport.bitmap, viewport.window);
                }
            }
        }
    }
